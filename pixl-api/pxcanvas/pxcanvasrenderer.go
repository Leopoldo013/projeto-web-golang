...
func (renderer *PxCanvasRenderer) LayoutCanvas(size fyne.Size) {
	...
	imgPxHeight := renderer.pxCanvas.PxRow
...
func (renderer *PxCanvasRenderer) LayoutBorder(size fyne.Size) {
	...
	botom := &renderer.canvasBorder[3]
	botom.Position1 = fyne.NewPos(offset.X, offset.Y+imgHeight)
	botom.Position2 = fyne.NewPos(offset.X+imgWidth, offset.Y+imgHeight)
