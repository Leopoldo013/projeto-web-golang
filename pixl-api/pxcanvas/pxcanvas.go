...
func NewBlqankImage(cols, row int, c color.Color) image.Image {
	img := image.NewNRGBA(image.Rect(0, 0, cols, row))
	for y := 0; y < row; y++ {
		for x := 0; x < cols; x++ {
			img.Set(x, y, c)
		}
...
func NewPxCanvas(state *apptype.State, config apptype.PxCanvasConfic) *PxCanvas {
	pxCanvas := &PxCanvas{
		PxCanvasConfic: config,
		appState:       state,
	}
	pxCanvas.PixelData = NewBlqankImage(config.PxCols, config.PxRow, color.NRGBA{128, 128, 128, 255})
...
func (pxCanvas *PxCanvas) LoadImage(img image.Image) {
	dimension := img.Bounds()

	pxCanvas.PxCanvasConfic.PxCols = dimension.Dx()
	pxCanvas.PxCanvasConfic.PxRow = dimension.Dy()
